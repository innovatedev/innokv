 ...
 * Fresh 2.0's default asset serving uses Deno.open with relative paths,
 * which fails when running from the JSR cache (jsr: scheme). We intercept these requests
 * and serve them from the local Deno cache or fallback to HTTPS.
 */
...
export async function jsrAssetBridge(ctx: FreshContext<any>) {
  ...
  const isJsr = import.meta.url.startsWith("jsr:") ||
    import.meta.url.startsWith("https://jsr.io");
  ...
  if (
    baseDir &&
    (baseDir.endsWith(".ts") || baseDir.endsWith(".js") ||
      baseDir.endsWith(".mjs"))
  ) {
  ...
  if (pathname.startsWith("/assets/")) {
    ...
    const jsrUrl =
      `https://jsr.io/@innovatedev/innokv/${APP_VERSION}/_fresh/client${pathname}`;
    try {
  ...
  if (staticFiles.includes(pathname)) {
    ...
    const jsrUrl =
      `https://jsr.io/@innovatedev/innokv/${APP_VERSION}/static${pathname}`;
    try {
      const resp = await fetch(jsrUrl);
      if (resp.ok) return resp;
    } catch { /* fallback */ }
